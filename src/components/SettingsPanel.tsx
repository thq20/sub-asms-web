...
export function SettingsPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const removeMaster = async (item?: Master, group?: string) => {
    const title = item ? `“${item.name}”` : `toàn bộ mục “${MASTER_LABEL[group || ""] || group}”`;
    if (!window.confirm(`Xóa ${title}? Danh mục đang được tài sản sử dụng sẽ được bảo vệ và không thể xóa.`)) return;
    try {
      setBusy(true);
      ...
      await loadMaster(); window.dispatchEvent(new Event("master-data-changed")); notify(`Đã xóa ${result.removed || 0} dữ liệu nền.`);
    } catch (error: any) { notify(error.message); } finally { setBusy(false); }
  };
