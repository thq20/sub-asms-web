...
export function createImportTemplate(master: Master[]) {
  ...
  const listEntries = Object.entries({ assetType: [...scopedSubTypes, ...names("ASSET_TYPE")], location: names("LOCATION"), floor: names("FLOOR"), owner: names("OWNER"), purchasingUnit: names("PURCHASING_UNIT"), status: ["UN_USED", "IN_USED", "MAINTENANCE", "BROKEN"], procurementCode: names("PROCUREMENT_CODE") });
  const headers = ["prefix", "assetType", "serialNumber", "asmsBarcode", "invoiceId", "procurementCode", "description", "originalCost", "location", "floor", "owner", "purchasingUnit", "seatCode", "ticketId", "status", "purchaseDate", "warrantyExpiry", "note"];
  ...
  const validationColumns: Record<string, number> = { B: 0, I: 1, J: 2, K: 3, L: 4, O: 5, F: 6 };
  ...
  const xml = {
    ...
    sheet1: `<?xml version="1.0" encoding="UTF-8"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView showGridLines="0" workbookViewId="0"/></sheetViews><sheetData><row r="1">${cells}</row></sheetData><dataValidations count="7">${validations}</dataValidations></worksheet>`,
